       ...
       function ground = finalize_init(ground, tile)
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            if ground.CHILD == 0  %CHILD does not exist
                ground = get_boundary_condition_u@GROUND_freezeC_RichardsEqW_Xice_seb(ground, tile); %call the native function for the ground class
...
%                     ground.CHILD = copy(CURRENT.STORE.SNOW);
        ...
        function ground = get_derivatives_prognostic(ground, tile)
            if ground.CHILD == 0  
                ground = get_derivatives_prognostic@GROUND_freezeC_RichardsEqW_Xice_seb(ground, tile); %call normal function
            else
        ...
        function timestep = get_timestep(ground, tile) 
            if ground.CHILD == 0
                timestep = get_timestep@GROUND_freezeC_RichardsEqW_Xice_seb(ground, tile);
            else 
