%========================================================================
% CryoGrid GROUND class GROUND_fcSimple_salt_seb_snow
...
classdef GROUND_fcSimple_salt_seb_snow < GROUND_fcSimple_salt_seb
    ...
    methods
        ...
        function ground = provide_CONST(ground)
            ground = provide_CONST@GROUND_fcSimple_salt_seb(ground);
        end
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            if ground.CHILD == 0  %SNOW CHILD does not exist
                ground = get_boundary_condition_u@GROUND_fcSimple_salt_seb(ground, tile); %call the native function for the ground class
...
%                     ground.CHILD = copy(CURRENT.STORE.SNOW);
                ...
                total_area = ground.STATVAR.area(1,1); %store the total area of the GROUND class
                ...
                ground = get_boundary_condition_u@GROUND_fcSimple_salt_seb(ground, tile);
        ...
        function [ground, S_up] = penetrate_SW(ground, S_down)  %mandatory function when used with class that features SW penetration
            [ground, S_up] = penetrate_SW@GROUND_fcSimple_salt_seb(ground, S_down);
        end
        
        function ground = get_boundary_condition_l(ground, tile)
            %forcing = tile.FORCING;
            ground = get_boundary_condition_l@GROUND_fcSimple_salt_seb(ground, tile);
        ...
        function ground = check_trigger(ground, tile)
            ...
            if ground.CHILD ~= 0
                %delete CHILD
                if ground.CHILD.STATVAR.area ./ ground.STATVAR.area(1,1) < 1e-6 %cutoff to delete remaining snow
                    ground.CHILD = 0;
                    ground.IA_CHILD = 0;
