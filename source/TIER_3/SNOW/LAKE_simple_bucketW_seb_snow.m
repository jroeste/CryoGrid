...
classdef LAKE_simple_bucketW_seb_snow < LAKE_simple_bucketW_seb
    ...
    methods
       ...
       function ground = finalize_init(ground, tile)
           ground = finalize_init@LAKE_simple_bucketW_seb(ground, tile);
           ground.CHILD = 0; % no snow
           ground.IA_CHILD = 0;
       end
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            if ground.CHILD == 0  %CHILD does not exist
                ground = get_boundary_condition_u@LAKE_simple_bucketW_seb(ground, tile); %call the native function for the ground class
...
%                     ground.CHILD = copy(CURRENT.STORE.SNOW);
                ...
                ground = get_boundary_condition_u@LAKE_simple_bucketW_seb(ground, tile);
        ...
        function ground = get_boundary_condition_l(ground, tile)
              ground = get_boundary_condition_l@LAKE_simple_bucketW_seb(ground, tile);
        end
        
        function [ground, S_up] = penetrate_SW(ground, S_down)  %mandatory function when used with class that features SW penetration
            [ground, S_up] = penetrate_SW@LAKE_simple_bucketW_seb(ground, S_down);
        end
        
        function ground = get_derivatives_prognostic(ground, tile)
            if ground.CHILD == 0  
                ground = get_derivatives_prognostic@LAKE_simple_bucketW_seb(ground, tile); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD, tile);
                ground = get_derivatives_prognostic@LAKE_simple_bucketW_seb(ground, tile); 
            end
        end
        
        function timestep = get_timestep(ground, tile) 
            if ground.CHILD == 0
                timestep = get_timestep@LAKE_simple_bucketW_seb(ground, tile);
            else 
                timestep_snow = get_timestep_CHILD(ground.CHILD, tile);
                timestep_ground =  get_timestep@LAKE_simple_bucketW_seb(ground, tile);
        ...
        function ground = advance_prognostic(ground, tile) 
            if ground.CHILD == 0
                ground =  advance_prognostic@LAKE_simple_bucketW_seb(ground, tile);
            else                
