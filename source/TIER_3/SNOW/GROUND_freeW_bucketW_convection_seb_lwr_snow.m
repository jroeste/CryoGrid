%========================================================================
% CryoGrid GROUND class GROUND_freeW_bucketW_convection_seb_lwr_snow
...
classdef GROUND_freeW_bucketW_convection_seb_lwr_snow < GROUND_freeW_bucketW_convection_seb_lwr
    properties
        CHILD
        IA_CHILD
    end
    ...
    methods
       ...
       function ground = provide_PARA(ground)  %initializes the subvariables as empty arrays
            ground = provide_PARA@GROUND_freeW_bucketW_convection_seb_lwr(ground);
       end
       
       function ground = provide_CONST(ground)  %initializes the subvariables as empty arrays
           ground = provide_CONST@GROUND_freeW_bucketW_convection_seb_lwr(ground);
       end
       
       function ground = provide_STATVAR(ground)  %initializes the subvariables as empty arrays
           ground = provide_STATVAR@GROUND_freeW_bucketW_convection_seb_lwr(ground);
       end
       
       function ground = finalize_init(ground, tile)
           ground = finalize_init@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile);
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            if ground.CHILD == 0  %CHILD does not exist
                ground = get_boundary_condition_u@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile); %call the native function for the ground class
...
%                     ground.CHILD = copy(CURRENT.STORE.SNOW);
                ...
                ground = get_boundary_condition_u@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile);
        ...
        function [ground, S_up] = penetrate_SW(ground, S_down)  %mandatory function when used with class that features SW penetration
            [ground, S_up] = penetrate_SW@GROUND_freeW_bucketW_convection_seb_lwr(ground, S_down);
        end
        
        function ground = get_boundary_condition_l(ground, forcing)
              ground = get_boundary_condition_l@GROUND_freeW_bucketW_convection_seb_lwr(ground, forcing);
        ...
        function ground = get_derivatives_prognostic(ground, tile)
            if ground.CHILD == 0  
                ground = get_derivatives_prognostic@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD, tile);
                ground = get_derivatives_prognostic@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile); 
            end
        end
        
        function timestep = get_timestep(ground, tile) 
            if ground.CHILD == 0
                timestep = get_timestep@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile);
            else 
                timestep_snow = get_timestep_CHILD(ground.CHILD, tile);
                timestep_ground =  get_timestep@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile);
        ...
        function ground = advance_prognostic(ground, tile) 
            if ground.CHILD == 0
                ground =  advance_prognostic@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile);
            else                
        ...
        function ground = compute_diagnostic(ground, tile)
            if ground.CHILD == 0
                ...
                ground = compute_diagnostic@GROUND_freeW_bucketW_convection_seb_lwr(ground, tile);
            else
