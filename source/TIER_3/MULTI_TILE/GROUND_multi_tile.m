...
classdef GROUND_multi_tile < SEB  %put all required ground classes here
    ...
    methods
        ...
        function ground = finalize_init(ground, tile)
            ...
            for ii = 1:size(ground.PARA.ground_classes,1)
                ...
                ground.SUB_TILES_TOP{ii,1} = Top();
                ground.SUB_TILES_TOP{ii,1}.NEXT = copy(tile.RUN_INFO.PPROVIDER.CLASSES.(ground.PARA.ground_classes{ii,1}){ground.PARA.ground_classes_index(ii,1)});
                ground.SUB_TILES_TOP{ii,1}.NEXT.PREVIOUS = ground.SUB_TILES_TOP{ii,1};
                ground.SUB_TILES_BOTTOM{ii,1} = Bottom();
                ground.SUB_TILES_TOP{ii,1}.NEXT.NEXT = ground.SUB_TILES_BOTTOM{ii,1};
                ground.SUB_TILES_BOTTOM{ii,1}.PREVIOUS = ground.SUB_TILES_TOP{ii,1}.NEXT;
                ...
                variables = fieldnames(ground.SUB_TILES_TOP{ii,1}.NEXT.STATVAR);
                ...
                for j=1:size(variables,1)
                    if isfield(GRID.STATVAR, variables{j,1})
                        ground.SUB_TILES_TOP{ii,1}.NEXT.STATVAR.(variables{j,1}) = GRID.STATVAR.(variables{j,1})(range);
                    end
                end
                ...
                ground.SUB_TILES_TOP{ii,1}.NEXT = convert_units(ground.SUB_TILES_TOP{ii,1}.NEXT, tile);
                ground.SUB_TILES_TOP{ii,1}.NEXT = finalize_init(ground.SUB_TILES_TOP{ii,1}.NEXT, tile);
                tile.PARA.area = total_area;
                ground.SUB_TILES_TOP{ii,1}.NEXT.PARA.target_grid = GRID.STATVAR.GRID;
                ground.SUB_TILES_TOP{ii,1}.NEXT.PARA.target_layerThick = GRID.STATVAR.layerThick;
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            for ii = 1:size(ground.SUB_TILES_TOP,1)
                ground.SUB_TILES_TOP{ii,1}.NEXT = get_boundary_condition_u(ground.SUB_TILES_TOP{ii,1}.NEXT, tile);
            end
...
%NEED TO THINK THIS THROUGH! PROBABLY NOT NEEDED FOR MOST CASES
        ...
        function ground = get_boundary_condition_l(ground, tile) %function will generally not be calles
            for ii = 1:size(ground.SUB_TILES_TOP,1)
%                 CURRENT = ground.SUB_TILES_TOP{ii,1}.NEXT;
%                 while ~strcmp(class(CURRENT.NEXT), 'Bottom')
%                     CURRENT= CURRENT.NEXT;
%                 end
                CURRENT = get_boundary_condition_l(ground.SUB_TILES_BOTTOM{ii,1}.PREVIOUS, tile);
            end
        end
