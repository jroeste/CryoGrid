...
classdef GROUND_multi_tile2_Cas < SEB  %put all required ground classes here
        ...
        function ground = get_boundary_condition_u(ground, tile)
            ...
            for ii = 1:size(ground.STATVAR.SUB_TILES_TOP,1)
                ...
                while ~strcmp(class(CURRENT), 'Bottom')
                    height = height + sum(CURRENT.STATVAR.layerThick,1);
                    CURRENT = CURRENT.NEXT;
                end
                height_of_class = [height_of_class; height];
            end
            average_height = mean(height_of_class,1);
            below_yes_no = height_of_class <= average_height - 0.05;
            receiving_fraction = sum(ground.PARA.areal_fractions .* double(below_yes_no));
            snow_fall_factor = [];
            for ii = 1:size(ground.STATVAR.SUB_TILES_TOP,1)
                if below_yes_no(ii,1)
                	snow_fall_factor = [snow_fall_factor; 1./ receiving_fraction];
                else
                    snow_fall_factor = [snow_fall_factor; 0 + double(sum(below_yes_no)==0)];
                end
            end

            for ii = 1:size(ground.STATVAR.SUB_TILES_TOP,1)
                orginial_snow_fall = tile.FORCING.TEMP.snowfall;
                tile.FORCING.TEMP.snowfall = tile.FORCING.TEMP.snowfall .* snow_fall_factor(ii,1);
                
                ground.STATVAR.SUB_TILES_TOP{ii,1}.NEXT = get_boundary_condition_u(ground.STATVAR.SUB_TILES_TOP{ii,1}.NEXT, tile);
                tile.FORCING.TEMP.snowfall = orginial_snow_fall;
            end
