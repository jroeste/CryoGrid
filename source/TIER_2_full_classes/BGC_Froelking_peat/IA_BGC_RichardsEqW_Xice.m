%========================================================================
% CryoGrid INTERACTION (IA) for BGC classes with
% GROUND_..._BGC clsses
% S. Westermann, November 2021
%========================================================================

classdef IA_BGC_RichardsEqW_Xice <  IA_BGC
    ...
    methods
        ...
        function send_BGC_variables(ia_BGC, tile)
            ...
            for i=1:size(ia_BGC.BGC.STATVAR.BGC_overlap_vector,1)
                ...
                d_organic(index, 1) = d_organic(index, 1) + ia_BGC.BGC.TEMP.delta_organic(i,1) .* ia_BGC.GROUND.STATVAR.area(index, 1);
                ia_BGC.GROUND.STATVAR.energy(index, 1) = ia_BGC.GROUND.STATVAR.energy(index, 1) + ia_BGC.BGC.TEMP.delta_organic(i,1) .* ia_BGC.GROUND.STATVAR.T(index, 1) .* ...
                    ia_BGC.GROUND.CONST.c_o .* ia_BGC.GROUND.STATVAR.area(index, 1);
                
                d_layerThick(index, 1) = d_layerThick(index, 1) + ia_BGC.BGC.TEMP.delta_layerThick(i,1);

            end

            %update organic
            ia_BGC.GROUND.STATVAR.organic = ia_BGC.GROUND.STATVAR.organic + d_organic;
            ia_BGC.GROUND.STATVAR.layerThick = ia_BGC.GROUND.STATVAR.layerThick + d_organic./ia_BGC.GROUND.STATVAR.area;
            ia_BGC.GROUND.STATVAR.layerThick_wo_Xice = ia_BGC.GROUND.STATVAR.layerThick_wo_Xice + d_organic./ia_BGC.GROUND.STATVAR.area;
            d_layerThick = d_layerThick - d_organic./ia_BGC.GROUND.STATVAR.area;
            ...
            d_air_pot = d_layerThick(plus_LT) .* ia_BGC.GROUND.STATVAR.area(plus_LT);
            ...
            ia_BGC.GROUND.STATVAR.layerThick(plus_LT) = ia_BGC.GROUND.STATVAR.layerThick(plus_LT) + d_air ./ ia_BGC.GROUND.STATVAR.area(plus_LT);
            ia_BGC.GROUND.STATVAR.layerThick_wo_Xice(plus_LT) = ia_BGC.GROUND.STATVAR.layerThick_wo_Xice(plus_LT) + (water_change + d_air) ./ ia_BGC.GROUND.STATVAR.area(plus_LT);

            %minus_LT, layerThick decreases, air is pressed out, if no air
            %available, waterIce is converted to XwaterIce  

            d_air_pot = -d_layerThick(minus_LT) .* ia_BGC.GROUND.STATVAR.area(minus_LT);
            d_air_max = max(0, ia_BGC.GROUND.STATVAR.layerThick(minus_LT) .* ia_BGC.GROUND.STATVAR.area(minus_LT) - ia_BGC.GROUND.STATVAR.XwaterIce(minus_LT) - ...
             ...
             ia_BGC.GROUND.STATVAR.layerThick(minus_LT) = ia_BGC.GROUND.STATVAR.layerThick(minus_LT) + d_air ./ ia_BGC.GROUND.STATVAR.area(minus_LT);
             ia_BGC.GROUND.STATVAR.layerThick_wo_Xice(minus_LT) = ia_BGC.GROUND.STATVAR.layerThick_wo_Xice(minus_LT) + (d_air - water_change) ./ ia_BGC.GROUND.STATVAR.area(minus_LT);
        ...
        function regrid_stratigraphy(ia_BGC, tile)
            ...
            if ~isempty(ia_BGC.BGC.STATVAR.BGC_overlap_vector)
                ...
                while i<=size(ia_BGC.BGC.STATVAR.BGC_overlap_vector,1)
                    ...
                    organic_BGC_remaining_fraction(index,1) = ia_BGC.BGC.STATVAR.total_peat(index,1) ./ ia_BGC.BGC.STATVAR.total_peat_originalMass(index,1);
...
%                         (waterIce_BGC(index,1) + XwaterIce_BGC(index,1)) ./ max(1e-40, sum(waterIce_BGC(index,1) + XwaterIce_BGC(index,1),1)) .* energy_waterIce(ia_BGC.BGC.STATVAR.BGC_overlap_vector(i,1),1);
                ...
                while i.*ia_BGC.GROUND.PARA.target_grid_cell_size<=layerThick_BGC_acc(end,1)
                    ...
                    soil_type_phys = [soil_type_phys; 1 + (4-1) .* double(mean(organic_BGC_remaining_fraction(start_pos:pos,1))<0.75)]; %soil type is 1 for less decomposed peat, 4 after
                ...
                ia_BGC.GROUND.STATVAR.soil_type = [soil_type_phys; ia_BGC.GROUND.STATVAR.soil_type(first_mineral_cell_old:end,1)];
                ia_BGC.GROUND.STATVAR.n = [double(soil_type_phys==1).* ia_BGC.GROUND.CONST.vanGen_n(1,1) + double(soil_type_phys==4).* ia_BGC.GROUND.CONST.vanGen_n(1,4); ia_BGC.GROUND.STATVAR.n(first_mineral_cell_old:end,1)];
                ia_BGC.GROUND.STATVAR.alpha = [double(soil_type_phys==1).* ia_BGC.GROUND.CONST.vanGen_alpha(1,1) + double(soil_type_phys==4).* ia_BGC.GROUND.CONST.vanGen_alpha(1,4); ia_BGC.GROUND.STATVAR.alpha(first_mineral_cell_old:end,1)];
                ia_BGC.GROUND.STATVAR.field_capacity = [energy_phys.*0 + 0.5; ia_BGC.GROUND.STATVAR.field_capacity(first_mineral_cell_old:end,1)];
                ia_BGC.GROUND.STATVAR.permeability = [energy_phys.*0 + 1e-13; ia_BGC.GROUND.STATVAR.permeability(first_mineral_cell_old:end,1)];
                ...
                ia_BGC.GROUND = calculate_hydraulicConductivity_RichardsEq_Xice2(ia_BGC.GROUND);
        ...
        function add_new_BGC_cells_from_READ(ia_BGC, tile)  %add the newly gained peat cells to first GROUND grid cell after switching from READ
           % make loop over entire overlap_vector
...
%                end
        ...
        function get_NPP_Frolking(ia_BGC, tile)
            ...
            ET_modifier = 1;%max(0, ia_BGC.GROUND.STATVAR.Qe ./ ia_BGC.GROUND.STATVAR.Qe_pot);
