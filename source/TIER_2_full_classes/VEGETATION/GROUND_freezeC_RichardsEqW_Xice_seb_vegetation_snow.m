...
classdef GROUND_freezeC_RichardsEqW_Xice_seb_vegetation_snow < GROUND_freezeC_RichardsEqW_Xice_seb_vegetation
    properties
        CHILD
        IA_CHILD
    end
    ...
    methods
       ...
       function ground = provide_PARA(ground)  
            ground = provide_PARA@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground);
       end
       
       function ground = provide_CONST(ground)  
           ground = provide_CONST@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground);
       end
       
       function ground = provide_STATVAR(ground)  
           ground = provide_STATVAR@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground);
       end
       
       function ground = finalize_init(ground, tile)
           ground = finalize_init@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile);
...
%                 disp('vegetation without snow');
                ground = get_boundary_condition_u@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile); %call the native function for the ground class
...
%                 disp('vegetation with snow');
        ...
        function [ground, S_up] = penetrate_SW(ground, S_down)  %mandatory function when used with class that features SW penetration
            [ground, S_up] = penetrate_SW@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, S_down);
        end
         
        function ground = get_boundary_condition_l(ground, tile)
              ground = get_boundary_condition_l@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile);
        end
        
        function ground = get_derivatives_prognostic(ground, tile)
            if ground.CHILD == 0  
                ground = get_derivatives_prognostic@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile); %call normal function
            else
                ground.CHILD = get_derivatives_prognostic_CHILD(ground.CHILD, tile);
                ground = get_derivatives_prognostic@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile); 
            end
        end
        
        function timestep = get_timestep(ground, tile) 
            if ground.CHILD == 0
                timestep = get_timestep@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile);
            else 
                timestep_snow = get_timestep_CHILD(ground.CHILD, tile);
                timestep_ground =  get_timestep@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile);
        ...
        function ground = advance_prognostic(ground, tile) 
            if ground.CHILD == 0
                ground =  advance_prognostic@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile);
            else                
        ...
        function ground = compute_diagnostic(ground, tile)
            if ground.CHILD == 0
                ground = compute_diagnostic@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile);
            else
...
% Turned around 20230805 Simone
        ...
        function ground = check_trigger(ground, tile)
            ...
            % Turned around 20230805 Simone
            ground = check_trigger@GROUND_freezeC_RichardsEqW_Xice_seb_vegetation(ground, tile);
