...
classdef SNOW_crocus2_bucketW_seb_vegetation < SNOW_crocus2_bucketW_seb
...
%             snow.PARA.slope = [];  %slope angle [-]
        ...
        %%% BEGIN CHANGES
        %%% 7 Nov 2023 Simone (adpated to
            % SNOW_crocus2_bucketW_seb.m)
        function snow = finalize_init(snow, tile) %assign all variables, that must be calculated or assigned otherwise for initialization
            snow = finalize_init@SNOW_crocus2_bucketW_seb(snow, tile);
...
%                 snow.PARA.airT_height = 2; 
%             end
            
%             if snow.VEGETATION == 0  %If vegetation does not exist
%             forcing = tile.FORCING;            
%             else
%             
%             end
%             
            ...
            %%% BEGIN CHANGES
            % 7 Nov 2023 Changed Simone (adpated to
            % SNOW_crocus2_bucketW_seb.m)
...
%             disp('snow_crocus2_CHILD');
            
%             if isfield(snow.PARENT, 'VEGETATION')
                forcing = snow.PARENT.VEGETATION.ForcingV; %            forcing = snow.PARENT.VEGETATION.ForcingV;
%             else
%                 forcing = tile.FORCING;
%             end
%             
%             if snow.PARENT.VEGETATION == 0  %If vegetation does not exists
%             forcing = tile.FORCING;            
%             else               
...
%             disp('snow_crocus2_create_CHILD');
%             if isfield(snow.PARENT, 'VEGETATION')
                forcing = snow.PARENT.VEGETATION.ForcingV; %            forcing = snow.PARENT.VEGETATION.ForcingV;
%             else
%                 forcing = tile.FORCING;
%             end
            ...
            snow.STATVAR.evap = 0; % SMS: maybe change to evaporation?
            snow.TEMP.evap_energy = 0; % % SMS: maybe change to evaporation?
            ...
            snow.TEMP.wind_d_D = 0;           
%             snow.TEMP.wind = FORCING.TEMP.wind;
            snow.TEMP.wind = tile.FORCING.TEMP.wind; % SMS: Why? different than SNOW_crocus2_bucketW_seb
            snow.TEMP.wind_surface = forcing.TEMP.wind;
...
%             if isfield(snow, 'VEGETATION')
            ...
            snow = get_boundary_condition_l@SNOW_crocus2_bucketW_seb(snow, tile);
%             else
%             snow = get_boundary_condition_l@SNOW_crocus2_bucketW_seb(snow, tile); 
%             end
        end
...
%             if isfield(snow, 'VEGETATION')
            ...
            snow = get_derivatives_prognostic@SNOW_crocus2_bucketW_seb(snow, tile);       
%             else
%             snow = get_derivatives_prognostic@SNOW_crocus2_bucketW_seb(snow, tile);            
%             end
        end
...
%             end
        ...
        function snow = advance_prognostic_CHILD(snow, tile)
            snow = advance_prognostic_CHILD@SNOW_crocus2_bucketW_seb(snow, tile);
        end
        
        function snow = compute_diagnostic_first_cell(snow, tile)
%             if isfield(snow, 'VEGETATION')
            snow.VEGETATION = compute_diagnostic_first_cell(snow.VEGETATION, tile);
            snow = compute_diagnostic_first_cell@SNOW_crocus2_bucketW_seb(snow, tile);
%             else
%             snow = compute_diagnostic_first_cell@SNOW_crocus2_bucketW_seb(snow, tile);
%             end
        end
...
%             if isfield(snow, 'VEGETATION')
            ...
            snow = compute_diagnostic@SNOW_crocus2_bucketW_seb(snow, tile);
%             else
%             snow = compute_diagnostic@SNOW_crocus2_bucketW_seb(snow, tile);
%             end
        end
        ...
        function snow = compute_diagnostic_CHILD(snow, tile)
            snow = compute_diagnostic_CHILD@SNOW_crocus2_bucketW_seb(snow, tile);
        end
...
%             else
%                 snow = check_trigger@SNOW_crocus2_bucketW_seb(snow, tile);            
% 
%             end
        end
