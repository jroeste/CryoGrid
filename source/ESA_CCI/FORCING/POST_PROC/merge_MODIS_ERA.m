...
classdef merge_MODIS_ERA < matlab.mixin.Copyable
    ...
    methods
        ...
        function post_proc = load_MODIS(post_proc, forcing, tile)
            ...
            retain_index = tile.PARA.key_internal - tile.PARA.key_internal(1,1) + 1; %index within chunk of (250) of the valid pixels
            
            fname = [post_proc.PARA.MODIS_path post_proc.PARA.MODIS_file '_' num2str(forcing.TEMP.current_year) '.nc'];
            
            MOD_LST_Day_1km = single(ncread(fname, 'MOD_LST_Day_1km', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf])).*0.02;
            MOD_LST_Day_1km = MOD_LST_Day_1km(retain_index, :);
            MOD_LST_Night_1km = single(ncread(fname, 'MOD_LST_Night_1km', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf])).*0.02;
            MOD_LST_Night_1km = MOD_LST_Night_1km(retain_index, :);
            MYD_LST_Day_1km = single(ncread(fname, 'MYD_LST_Day_1km', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf])).*0.02;
            MYD_LST_Day_1km = MYD_LST_Day_1km(retain_index, :);
            MYD_LST_Night_1km = single(ncread(fname, 'MYD_LST_Night_1km', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf])).*0.02;
            MYD_LST_Night_1km = MYD_LST_Night_1km(retain_index, :);

            MOD_Day_view_time = double(ncread(fname, 'MOD_Day_view_time', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf]));
            MOD_Day_view_time = MOD_Day_view_time(retain_index, :);
            MOD_Day_view_time(MOD_Day_view_time==255) = NaN;
            MOD_Night_view_time = double(ncread(fname, 'MOD_Night_view_time', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf]));
            MOD_Night_view_time = MOD_Night_view_time(retain_index, :);
            MOD_Night_view_time(MOD_Night_view_time == 255) = NaN;
            MYD_Day_view_time = double(ncread(fname, 'MYD_Day_view_time', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf]));
            MYD_Day_view_time = MYD_Day_view_time(retain_index, :);
            MYD_Day_view_time(MYD_Day_view_time==255) = NaN;
            MYD_Night_view_time = double(ncread(fname, 'MYD_Night_view_time', [tile.PARA.key_internal(1,1) 1], [tile.PARA.key_internal(end,1)-tile.PARA.key_internal(1,1)+1 Inf]));
            MYD_Night_view_time = MYD_Night_view_time(retain_index, :);
            MYD_Night_view_time(MYD_Night_view_time==255) = NaN;
            
%             MOD_LST_Day_1km = single(ncread(fname, 'MOD_LST_Day_1km', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf])).*0.02;
%             MOD_LST_Night_1km = single(ncread(fname, 'MOD_LST_Night_1km', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf])).*0.02;
%             MYD_LST_Day_1km = single(ncread(fname, 'MYD_LST_Day_1km', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf])).*0.02;
%             MYD_LST_Night_1km = single(ncread(fname, 'MYD_LST_Night_1km', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf])).*0.02;
%             
%             MOD_Day_view_time = double(ncread(fname, 'MOD_Day_view_time', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf]));
%             MOD_Day_view_time(MOD_Day_view_time==255) = NaN;
%             MOD_Night_view_time = double(ncread(fname, 'MOD_Night_view_time', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf]));
%             MOD_Night_view_time(MOD_Night_view_time == 255) = NaN;
%             MYD_Day_view_time = double(ncread(fname, 'MYD_Day_view_time', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf]));
%             MYD_Day_view_time(MYD_Day_view_time==255) = NaN;
%             MYD_Night_view_time = double(ncread(fname, 'MYD_Night_view_time', [tile.PARA.range(1) 1], [size(tile.PARA.range,1) Inf]));
%             MYD_Night_view_time(MYD_Night_view_time==255) = NaN;
            ...
            MOD_Day_view_time = SolarTime2LocalTime(post_proc, MOD_Day_view_time, doy, tile.PARA.longitude, 0);
            MOD_Night_view_time = SolarTime2LocalTime(post_proc, MOD_Night_view_time, doy, tile.PARA.longitude, 0);
            MYD_Day_view_time = SolarTime2LocalTime(post_proc, MYD_Day_view_time, doy, tile.PARA.longitude, 0);
            MYD_Night_view_time = SolarTime2LocalTime(post_proc, MYD_Night_view_time, doy, tile.PARA.longitude, 0);
            
            post_proc.TEMP.MODIS_LST = [MOD_LST_Day_1km MOD_LST_Night_1km MYD_LST_Day_1km MYD_LST_Night_1km];
            post_proc.TEMP.MODIS_LST_time = [MOD_Day_view_time MOD_Night_view_time MYD_Day_view_time MYD_Night_view_time];
