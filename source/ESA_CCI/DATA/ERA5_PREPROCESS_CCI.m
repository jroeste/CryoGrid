...
classdef ERA5_PREPROCESS_CCI < FORCING_MULTITILE_seb_CCI %FORCING_seb_CCI_full
            ...
            for i = era.PARENT.RUN_INFO.PARA.worker_number:era.PARENT.RUN_INFO.PARA.number_of_cores:size(year_list,1)
                ...
                while fail_count < 100
                    try
                        ...
                        if variable_list(i,1) == 1
                            delete([era.PARA.target_path filename_SURF_airT])
                        end
                        if variable_list(i,1) == 2
                            delete([era.PARA.target_path filename_SURF_precip])
                        end
                        if variable_list(i,1) == 3
                            delete([era.PARA.target_path filename_PLEV_airT])
                        end
                        if variable_list(i,1) == 4
                            delete([era.PARA.target_path filename_PLEV_geopotential])
                        end
                        if variable_list(i,1) == 5
                            delete([era.PARA.target_path filename_SURF_dewpointT])
                        end
                        if variable_list(i,1) == 6
                            delete([era.PARA.target_path filename_SURF_solar_radiation])
                        end
                        if variable_list(i,1) == 7
                            delete([era.PARA.target_path filename_SURF_thermal_radiation])
                        end
                        if variable_list(i,1) == 8
                            delete([era.PARA.target_path filename_SURF_TOA_radiation])
                        end
                        if variable_list(i,1) == 9
                            delete([era.PARA.target_path filename_PLEV_RH])
                        end
