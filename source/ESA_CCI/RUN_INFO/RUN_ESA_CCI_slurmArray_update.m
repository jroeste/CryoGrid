...
classdef RUN_ESA_CCI_slurmArray_update < matlab.mixin.Copyable
    ...
    methods
        ...
        function run_info = provide_PARA(run_info)
            ...
            run_info.PARA.assign_run_info_properties_class = [];
            run_info.PARA.assign_run_info_properties_class_index = [];
        ...
        function run_info = finalize_init(run_info)
            ...
            %choose the correct MODIS_deg tile either automatically or by
            %choosing the number in the list
            if ~isempty(run_info.PARA.assign_run_info_properties_class) && sum(isnan(run_info.PARA.assign_run_info_properties_class)) == 0
                update = copy(run_info.PPROVIDER.CLASSES.(run_info.PARA.assign_run_info_properties_class){run_info.PARA.assign_run_info_properties_class_index,1});
                update = finalize_init(update);
                update = assign_run_info_properties(update, run_info);
                run_info.TEMP.update = update;
            end
        ...
        function [run_info, tile] = run_model(run_info)
            ...
            if ~run_info.PARA.preprocess_only %do nothing if this variable is set, can be used to enable preprosessing  with batch script and "wait" command
                ...
                if ~isempty(run_info.PARA.assign_run_info_properties_class) && sum(isnan(run_info.PARA.assign_run_info_properties_class)) == 0
                    run_info.TEMP.update = finalize_progress_list(run_info.TEMP.update, run_info);
                end
            end
        end
