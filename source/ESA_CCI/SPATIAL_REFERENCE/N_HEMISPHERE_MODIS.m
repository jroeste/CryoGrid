...
classdef N_HEMISPHERE_MODIS < matlab.mixin.Copyable
    ...
    methods
        function proj = provide_PARA(proj)
            ...
            proj.PARA.MODIS_filename = [];
            proj.PARA.MODIS_folder = [];
        ...
        function proj = finalize_init(proj)
            ...
            proj.STATVAR.key_tile = [];
            proj.STATVAR.tile_number = [];
            for i=1:size(MODIS_deg_list,1)
                lat = ncread([proj.PARA.MODIS_folder proj.PARA.MODIS_filename '_' num2str(MODIS_deg_list(i,1)) '_' num2str(MODIS_deg_list(i,2)) '_' num2str(MODIS_deg_list(i,3)) '_' num2str(MODIS_deg_list(i,4)) '_2003.nc'], 'latitude');
                lon = ncread([proj.PARA.MODIS_folder proj.PARA.MODIS_filename '_' num2str(MODIS_deg_list(i,1)) '_' num2str(MODIS_deg_list(i,2)) '_' num2str(MODIS_deg_list(i,3)) '_' num2str(MODIS_deg_list(i,4)) '_2003.nc'], 'longitude');
                key_tile = ncread([proj.PARA.MODIS_folder proj.PARA.MODIS_filename '_' num2str(MODIS_deg_list(i,1)) '_' num2str(MODIS_deg_list(i,2)) '_' num2str(MODIS_deg_list(i,3)) '_' num2str(MODIS_deg_list(i,4)) '_2003.nc'], 'key');
                proj.STATVAR.latitude = [proj.STATVAR.latitude; lat];
                proj.STATVAR.longitude = [proj.STATVAR.longitude; lon];
                proj.STATVAR.key_tile = [proj.STATVAR.key_tile; key_tile];
                proj.STATVAR.tile_number = [proj.STATVAR.tile_number; key_tile .* 0 + i];
            end
            ...
            %order in longitudinal direction, but for blocks of 5 degrees
            lat_lon=[round(proj.STATVAR.latitude/5)*5 round(proj.STATVAR.longitude/5)*5];
            [~, order]=sortrows(lat_lon,[2 1]);
            fn = fieldnames(proj.STATVAR);
            for i=1:size(fn,1)
                proj.STATVAR.(fn{i,1}) = proj.STATVAR.(fn{i,1})(order,:);
            end
