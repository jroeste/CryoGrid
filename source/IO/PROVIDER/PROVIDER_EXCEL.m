classdef PROVIDER_EXCEL < BASE_PROVIDER
        ...
        function provider = read_const_excel(provider)
            ...
            for i=1:size(data,1)
                if ~isnan(data{i,1}) & ~isnan(data{i,2})
                    provider.CONST.(data{i,1}) = data{i,2};
                end
            end
        end
...
%                             end
        ...
        %parallel runs
        function provider = update_parameter_file_excel(provider, param_file_number)
        ...
        function provider = update_run_name_excel(provider, worker_number)
