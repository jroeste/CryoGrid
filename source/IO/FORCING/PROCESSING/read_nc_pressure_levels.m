...
classdef read_nc_pressure_levels < matlab.mixin.Copyable 
    ...
    methods
        function proc = provide_PARA(proc)
            ...
            proc.PARA.top_pressure_level = [];
            proc.PARA.bottom_pressure_level = [];
        end
        ...
        function forcing = process(proc, forcing, tile)
            ...
            p = proc.PARA.top_pressure_level:25:proc.PARA.bottom_pressure_level;%[800 825 850 875 900 925 950 975 1000]; era.p=p.*1e2; % Store in Pascals.
            era.p=[];
            ...
            for l=1:numel(p)
                if strcmp(proc.PARA.time_resolution_input, 'quarter')
                    ncf=[proc.PARA.forcing_path proc.PARA.nc_folder 'plev_q' num2str(quarter) '_p' num2str(p(l)) '_' num2str(year) '.nc'];
                elseif strcmp(proc.PARA.time_resolution_input, 'month')
                    ncf=[proc.PARA.forcing_path proc.PARA.nc_folder 'plev_m' num2str(month) '_p' num2str(p(l)) '_' num2str(year) '.nc'];
                elseif strcmp(proc.PARA.time_resolution_input, 'year')
                    ncf=[proc.PARA.forcing_path proc.PARA.nc_folder 'plev_p' num2str(p(l)) '_' num2str(year) '.nc'];
                end
                if exist(ncf)==2
                    Z=ncread(ncf,'z')./9.81;
                    era.Z(:,:,ind,:)=Z;
