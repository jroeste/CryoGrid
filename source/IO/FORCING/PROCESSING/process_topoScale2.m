%========================================================================
% CryoGrid FORCING processing class process_topoScale2
...
classdef process_topoScale2 < process_BASE
...
%             era_q_sl=wsl./(1+wsl);
            era_q_sl = eps0.*vpsl./era_p_sl;
                        
            era_RH_sl = Magnus(era_Td_sl) ./ Magnus(double(era_T_sl).*era.T_sf);
            ...
            for i=2:size(era.Z,3)
                layer_below(:,i,:) = double(era_alt(:,i,:) < forcing.SPATIAL.STATVAR.altitude & era_alt(:,i-1,:) >= forcing.SPATIAL.STATVAR.altitude & era_alt(:,i,:) > era_alt_sl);
                layer_above(:,i-1,:) = double(era_alt(:,i,:) < forcing.SPATIAL.STATVAR.altitude & era_alt(:,i-1,:) >= forcing.SPATIAL.STATVAR.altitude & era_alt(:,i-1,:) > era_alt_sl);
            end
            
            distance_Z_above = abs(sum(era_alt .* layer_above ,2) - forcing.SPATIAL.STATVAR.altitude) .* double(sum(layer_above,2) > 0);
            distance_Z_below = abs(sum(era_alt .* layer_below ,2) - forcing.SPATIAL.STATVAR.altitude) .* double(sum(layer_below,2) > 0);
            ...
            use_sl = sum(weights_Z_above + weights_Z_below,2) <1-1e-9  | forcing.SPATIAL.STATVAR.altitude < era_alt_sl;
            ...
            RH_topoScale_sl = era_RH_sl .* double(weights_lat);
            ...
            %pressure
            for i=2:size(era.Z,3)
                layer_below(:,i,:) = double(era_alt(:,i,:) < forcing.SPATIAL.STATVAR.altitude & era_alt(:,i-1,:) >= forcing.SPATIAL.STATVAR.altitude);
                layer_above(:,i-1,:) = double(era_alt(:,i,:) < forcing.SPATIAL.STATVAR.altitude & era_alt(:,i-1,:) >= forcing.SPATIAL.STATVAR.altitude);
            end
            ...
            p_topoScale(p_topoScale==0) = era_p_sl(p_topoScale==0) .* exp(-g.*M.*(forcing.SPATIAL.STATVAR.altitude - era_alt_sl(p_topoScale==0))./R_gc./(double(era_T_sl(p_topoScale==0)).*era.T_sf + 273.15));
            ...
            %Sin
            [solar_azm, solar_zen]=solargeom(proc, era.t, forcing.SPATIAL.STATVAR.latitude, forcing.SPATIAL.STATVAR.longitude);
            ...
            era_S_TOA_point = S0.*mu0;
            
            kt=era_kd; %era_Sin_sl./max(1e-10, era_S_TOA_sl); % Clearness index.
            kd=0.952-1.041.*exp(-1.*exp(2.3-4.702.*kt)); % Diffuse fraction.
            kd=max(kd,0);
            
            % Diffuse component.
            era_Sin_sl = era_kd .* era_S_TOA_point;
            ...
            Sin_dir_topoScale = era_S_TOA_point.*(Sin_dir_topoScale./(max(1e-10, era_S_TOA_point))).^(p_topoScale./era_p_sl);
            ...
            dZ=forcing.SPATIAL.STATVAR.altitude - era_alt_sl2; % m
            dZ=dZ./1e3; % km
            ...
            forcing.DATA.q = eps0.*RH_topoScale_sl.*Magnus(double(T_topoScale))./double(p_topoScale);
