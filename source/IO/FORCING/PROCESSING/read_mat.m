...
classdef read_mat < matlab.mixin.Copyable 
    ...
    methods
        ...
        function forcing = process(proc, forcing, tile)
            ...
            if isfield(temp, 'time')
                variables = fieldnames(temp.time);
                for i=1:size(variables, 1)
                    forcing.DATA.(variables{i,1}) = temp.time.(variables{i,1});
                end
            end
            if isfield(temp, 'spatial')
                variables = fieldnames(temp.spatial);
                for i=1:size(variables, 1)
                    forcing.DATA.(variables{i,1}) = temp.spatial.(variables{i,1});
                end
            end
            ...
            for i=1:size(variables,1)
                if isfield(temp, variables{i,1}) && ~strcmp(variables{i,1}, 't_span') && ~strcmp(variables{i,1}, 'timeForcing')
                    forcing.DATA.(variables{i,1}) = squeeze(temp.(variables{i,1}));
                end
            end
            if isfield(temp, 't_span')
                forcing.DATA.timeForcing = temp.t_span;
            elseif isfield(temp, 'timeForcing')
               forcing.DATA.timeForcing = temp.timeForcing;
            end
