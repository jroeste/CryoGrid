...
classdef OUT_TwaterIce < matlab.mixin.Copyable
        ...
        function out = store_OUT(out, tile)           
            ...
            if t>=out.OUTPUT_TIME
                ...
                if t>=out.SAVE_TIME
                    ...
                    OUT_TwaterIce = out.result;
                    OUT_TwaterIce.timestamp = out.timestamp;
                    if isempty(out_tag) || all(isnan(out_tag))
                        save([result_path run_name '/' run_name '_TwaterIce_' datestr(t,'yyyymmdd') '.mat'], 'OUT_TwaterIce')
                    else
                        save([result_path run_name '/' run_name '_TwaterIce_' out_tag '_' datestr(t,'yyyymmdd') '.mat'], 'OUT_TwaterIce')
                    end
