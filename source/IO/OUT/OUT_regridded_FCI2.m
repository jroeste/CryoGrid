...
classdef OUT_regridded_FCI2 < matlab.mixin.Copyable
    ...
    methods
        ...
        function out = provide_PARA(out)         
            ...
            out.PARA.variables = [];
            out.PARA.height_above_ground = [];
            out.PARA.depth_below_ground = [];
        ...
        function out = finalize_init(out, tile)
            ...
            out.PARA.upper_elevation = tile.PARA.altitude + out.PARA.height_above_ground;
            out.PARA.lower_elevation = tile.PARA.altitude - out.PARA.depth_below_ground;
...
%         function out = store_OUT(out, t, TOP, BOTTOM, forcing, run_number, timestep, result_path)
        ...
        function out = regrid_out(out, result)
            ...
            for j=1:size(result,1)
                ...
                water =[water; result{j,1}.STATVAR.water ./ result{j,1}.STATVAR.layerThick ./ result{j,1}.STATVAR.area];
                T =[T; result{j,1}.STATVAR.T];
                try
                    porosity = [porosity; 1 - (result{j,1}.STATVAR.mineral + result{j,1}.STATVAR.organic) ./ result{j,1}.STATVAR.layerThick ./ result{j,1}.STATVAR.area];
                catch
                    porosity = [porosity; result{j,1}.STATVAR.T .*0 + 1];
                end
            end
            
            elevations =  cumsum(layerThick);
            elevations = elevations(end)- elevations + altitudeLowestCell + layerThick/2;

            
            FCI = getFCI(out, T, water, porosity, layerThick);
            
            out.FCI = [out.FCI interp1(elevations, FCI, new_grid, 'nearest')];
        end
        

        function FCI = getFCI(out, T, water, porosity, layerThick)
            
            unfrozenWater = water;
            
            is_bedrock  = porosity < 0.05;
            
            
            %1. frost cracking window and not snow
            FCI = double(T>=-8 & T<=-3 & porosity <0.95) ; 
            
            
            %2. water availablity
            
            flow_restriction_warm_sed = 1; % m^-1
            flow_restriction_cold_sed = 2; % m^-1
            flow_restriction_warm_rock = 2;% m^-1
            flow_restriction_cold_rock = 4; % m^-1  %from Andersen et al. 2015
            
            flow_restriction = flow_restriction_warm_sed .* double(~is_bedrock) .* double(T>0) + flow_restriction_cold_sed .* double(~is_bedrock) .* double(T<=0) + ...
                flow_restriction_warm_rock .* double(is_bedrock) .* double(T>0) + flow_restriction_cold_rock .* double(is_bedrock) .* double(T<=0);
            ...
            for i=1:size(T,1)
                if ~(FCI(i,m) == 0)
                    %direction of flow
                    if i>1
                        upwards_warmer = T(i-1,m)>T(i,m);
                    else
                        upwards_warmer = 0;
                    end
                    if i<size(T,1)
                        downwards_warmer = T(i,m)<T(i+1,m);
                    else
                        downwards_warmer = 0;
                    end
                    FCI_value = 0;
                    if upwards_warmer
                        for j=i-1:1
                            flow_restr = sum(flow_restriction(j:i-1,m) .* layerThick(j:i-1,m));
                            FCI_value = FCI_value + exp(-flow_restr) .* unfrozenWater(j,m);
                        end
                    end
                    if downwards_warmer
                        for j=i+1:size(T,1)
                            flow_restr = sum(flow_restriction(i+1:j,m) .* layerThick(i+1:j,m));
                            FCI_value = FCI_value + exp(-flow_restr) .* unfrozenWater(j,m);
                        end
                    end
                    FCI(i,m) = FCI_value;
            ...
            gradient = abs((T(1:end-1,:)-T(2:end,:)) ./ (layerThick(1:end-1,:)./2 + layerThick(2:end,:)./2));
            
            gradient2 = T.*0;
            gradient2(1,:) = gradient(1,:);
            gradient2(end,:) = gradient(end,:);
            gradient2(2:end-1,:) = gradient(1:end-1,:)./2 + gradient(2:end,:)./2;
