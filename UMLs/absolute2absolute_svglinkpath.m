%==========================================================================
% Converts links in svg files from a wrong absolute path (from a different
% installation) to the correct absolute path according to the current 
% install location.
% T. Ingemann-Nielsen, November 2020
%==========================================================================

function absolute2absolute_svglinkpath(graphic_file)
    ...
    for m = 1:length(s)
        ...
        for n = 1:length(matches)
            ...
            if ~startsWith(pstr, basepath)
                % Get the relative path to the class, starting with the
                % modules directory
                relpath = pstr(strfind(pstr, '\modules\'):end);
                % Now replace the wrong part with the full path to the
                % modules directory and regenerate the full link
                % information
                mstr2 = strrep(mstr,tokens{n}{1}, ['''' basepath relpath '''']);
